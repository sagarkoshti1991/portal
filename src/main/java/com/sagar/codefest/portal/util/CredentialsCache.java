...
/**
 * Holds access tokens with an associated validity timestamp. The intention is
 * to minimize the number of calls to Cognito. Tokens should be added to the
 * cache on successful authentication or refresh. They will time out after 15
 * minutes, at which point the servlet must authenticate again.
 * <p>
 * To further minimize calls, a single cache should be injected into all
 * servlets.
 * <p>
 * Implementation notes:
 * <ul>
 * <li>This is an LRU cache based on <code>LinkedHashMap</code>. It's
 * constructed with the maximum number of retained entries, which should be
 * large enough to support expected use but small enough to avoid straining
 * memory.
 ...
 * <li>The cache is naively syncrhonized. In normal use this should be
 * sufficient and cause minimal contention. In high-volume use, consider
 * replacing with a <code>ConcurrentHashMap</code> and reaper thread.
 ...
 * it would never be serialized, as the servlet would never be passivated). 
 * </ul>
 */
public class CredentialsCache implements Serializable {
	...
	/**
	 * Adds an access token to the cache with specified timeout (in millis). This
	 * should be called when an uncached token has been validated (which would
	 * happen when the app restarts).
	 */
